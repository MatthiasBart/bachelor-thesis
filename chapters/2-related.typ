...
Although a lot of research exists on #gls("d2d") communication in cellular networks, most of it is done in a military use case @gamboa_system-level_2023, like unmanned aerial vehicles (UVA)  or public safety networks @gamboa_towards_2024, like vehicle to everything (V2X). Most of this research builds upon 5G New Radio (NR) Sidelink (SL) which has implemented protocol support for (V2X) and Proximity Services (ProSe) for public safety networks which allows #gls("ue") to directly talk to each other without the interference of a #gls("gnb").
