...
Although a lot of research exists on D2D communication in cellular networks, most of it is done in a military use case @Gamboa_Ben_Mosbah_Garey_Liu_Rouil_2023, like unmanned aerial vehicles (UVA)  or public safety networks @Gamboa_Henderson_Garey_Liu_Rouil_2024, like vehicle to everything (V2X). Most of this research builds upon 5G New Radio (NR) Sidelink (SL) which has implemented protocol support for vehicle to everything and Proximity Services (ProSe) for public safety networks which allows user equipment to directly talk to each other without the interferance of a base station (gNB).
Although approaches existed to also introduce D2D communication to the commercial markets back in 2014 by Qualcomm and #cite(<Condoluci_Militano_Orsino_Alonso-Zarate_Araniti_2015>, form: "author") back in 2015 proved that LTE-Direct, which preceded 5G SL, has some energy and scaling benefits over WiFi-Direct, no support for this technology is given on mobile smartphones Eskimo answer. Like also pointet out by the authors of this critical review of mobile device-to-device communication @Desauw_Luxey-Bitri_Raes_Rouvoy_Ruas_Rudametkin_2023.

== Apple Ecosystem and TU Darmstadt

As already mentioned, the OWL project by Secure Mobile Networking Lab (SEEMOO) by TU darmstadt contributed several papers to research on Apple's wireless ecosystem @Stute_Kreitschmann_Hollick_2018a. Their goal was to assess security and privacy as well asw enable cross-platform compatibility with other vendors. They started to investigate AWDL in an approach to reverse engineere the 802.11 Wifi based protocol and 
developed an open source application that implements Apples AWDL. The authors used binary and runtime analyses to reconstruct the daemons and frameworks involved in communicating via AWDL and found that each AWDL node announces a sequence of Availability Windows indicationg that it is ready to communicate with other AWDL nodes. In the process they als found several security issues which allowed an attacker to access the macOS WiFi dirver interface which could lead to DoS attacks @Stute_Kreitschmann_Hollick_2018b.

In 2019 and 2021 after finding the first vulnerabilities when reverse engineering the AWDL protocol they dedicated a seperate paper on researching security and privacy issues in this protocol. The study uncovers multiple vulnerabilities related to both design flaws and implementation bugs. One of the major findings is the possibility of a man-in-the-middle (MitM) attack, which allows an attacker to stealthily modify files transferred via AirDrop. Additionally, the study identifies denial-of-service (DoS) vulnerabilities that can disrupt communication or force the sudden crash of all nearby devices. The research also reveals privacy weaknesses that allow attackers to track users over extended periods, effectively bypassing MAC address randomization. The vulnerabilities were discovered through a combination of reverse engineering, code analysis, and patent examination. To demonstrate the feasibility of these attacks, the researchers developed proof-of-concept implementations using inexpensive hardware, including a 20 dollar micro:bit device. Following responsible disclosure, Apple addressed one of the DoS attack vulnerabilities. However, the researchers highlight that several of the identified security and privacy risks require fundamental redesigns of some of Apple’s services to be fully mitigated. The study ultimately highlights critical security flaws in AWDL, demonstrating the potential impact on over a billion Apple devices and emphasizing the need for stronger security measures and protocol improvements Onebillionopeninterfaces Disrupting continuity.

In 2020 Ian Beer a british computer securtiy expert and white hat hacker, inspired by the work of TU Darmstadt found another severe security issue in AWDL which could remotely trigger an unauthenticated kernel memory corruption that lead to all iOS devices in radio proximity to reboot. He also describes how this issue can lead to a vulnerability that lets an attacker run any code on nearby iOS devices and steal all user information. atIanBeer_2020
