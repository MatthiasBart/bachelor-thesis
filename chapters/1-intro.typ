...
Nowadays many different technologies exist to gain access to wide area ad-hoc connections like LTE-Direct, 5G #gls("sl") or LoraWan. Unfortunately these technologies are not yet supported in smartphones and there is no guarantee that they will ever be. However modern smartphones improved support for local #gls("ptp") technologies in recent years, that allow devices to directly communicate via local radio link broadcasts. This could improve connectivity in the aforementioned cases or enable scenarios in which numerous end devices connect together, to form a wide spread mesh network. The first technology that comes to mind is Bluetooth. First introduced in 1998 it has already been used in mobile computing the last decades, but due to limited data transfer rates a new approach started to enter the market. These days #gls("wifi") is predominantly used to facilitate local #gls("ptp") connections between smartphones, whereby unfortunately every mobile operating system has created its own version which makes communication between different vendors complex. Furthermore, the extent to which these technologies can provide stable data transfer and the quality with which they can do so, remains unclear or poorly documented. Since this information is essential for a further evaluation of the feasibility of applications based on direct #gls("ptp") connections, this thesis aims to address the existing uncertainty by measuring metrics that can assist the evaluation of analogous applications. 

//This explains why i am using iOS and what I am using
As of 2024 4 billion people worldwide are carrying smartphones @statista_research_department_number_2024 with them. Twenty seven percent of them are developed and manufactured by one of the most valuable companies in the world. Since 2007, when the first iPhone was released on 27th of June Apple has sold more than 2.8 billion devices @sherif_apple_2025. This makes iOS one of the most used mobile operating systems worldwide with a current market share of 27% @ahmed_sherif_market_2025. iOS also utilizes its own implementation of a #gls("ptp") WiFi protocol which is strongly used by Apples Continuity @apple_inc_continuity_2024 which bundles applications like AirDrop or AirPlay, Universal Clipboard, Handoff or WiFi password sharing for contacts that want to join your network. 

Apples #gls("ptp") WiFi implementation is called #gls("awdl") and is an undocumented protocol that is based on the 802.11 ad-hoc WiFi standard which did not live up to its expectations. This protocol is not only used internally by Continuity but is also accessible to iOS application developers via various frameworks that operate on different layers of abstraction. 

Although Bluetooth is more versatile, compatible with devices from other vendors and has improved range and speed in mobile end devices in recent years, #gls("awdl") is the recommended technology to establish direct links between iOS devices @apple_inc_peer--peer_2023.
...
After all the described aspects have been taken into consideration, the Networking Framework, included in the iOS #gls("sdk") is used to establish and test the quality of #gls("ptp") #gls("awdl") connections between iOS devices. The tests will alternate using TCP, UDP and QUIC in different surroundings that represent typical locations, varying in crowd density or general ambient #gls("rfemf") levels.

== Research Definition

This research should help assess feasibility of applications that want to utilize #gls("awdl"). It will compare different transport layer protocols (TCP, UDP and QUIC) in different locations that represent common places (City, Underground, Field and Forrest) to cover various characteristics of real life scenarios. The tests will vary in distance, size of packages and number of packages. The Networking Framework is used to quantify the connection quality through metrics like #gls("rtt"), Jitter, package loss and data transfer speed.

*Research Questions*

Which aspects influence the #gls("ptp") #gls("awdl") connection quality on iOS devices and how far can it reach?

*Hypothesis*

$H_1$

#gls("ptp") #gls("awdl") connection quality on iOS devices depends on the surroundings and functions worse in crowded areas.

$H_2$

#gls("ptp") #gls("awdl") connection quality on iOS devices depends on the transport layer protocol.
...
Nowadays Smartphones mostly rely on infrastructure networks. As this is a strong dependency that could vastly limit access and advancements in #gls("ptp") connection soft- and hardware have emerged in recent years it is unclear which connection qualities these methods can produce. Therefore a prototype application developed for the iOS platform serves as a utility for measuring  selected metrics.
...
// peer-to-peer, manet, wifi direct, #gls("awdl"), ad-hoc, 
