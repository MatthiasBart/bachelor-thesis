...
Moreover Apple provides the Network framework @apple_inc_network_nodate which can directly access #gls("awdl") and the underlying #gls("ips") Transport Layer. The Transport Layer is composed of the protocols #gls("tcp"), #gls("udp") and recently also #gls("quic") which Apple also added support for starting from #gls("ios") 15.
